# master.py — JobFlow AI (cloud mini)
...
for p in (OUTPUT, SESSIONS, UPLOADS): p.mkdir(parents=True, exist_ok=True)
...
def get_payload(sid: str):
    ...
    if gdir.exists():
        for p in sorted(gdir.iterdir()):
            if p.is_file(): imgs.append({"filename": p.name, "url": f"/uploads/{sid}/{p.name}"})
    return {
        "session_id": sid,
...
def get_session(sid):
    sdir = SESSIONS / sid
    if not sdir.exists(): return jsonify({"error":"not_found"}), 404
...
def patch_session(sid):
    ...
    sdir = ensure_session_dirs(sid)
    data = _normalize(_read(sdir / "report.json"))
    if "client_name" in body: data["client_name"] = (body.get("client_name") or "").strip()
    if "job_type" in body:    data["job_type"]    = (body.get("job_type") or "").strip()
    if "notes" in body:       data["notes"]       = body.get("notes") or ""
...
def serve_upload(sid, filename):
    ...
    path = udir / filename
    if path.exists(): return send_from_directory(udir, filename)
    abort(404)
...
#flash{position:fixed;right:16px;top:16px;padding:10px 14px;border-radius:8px;display:none;background:#22c55e;color:#04190a}
...
<div class=wrap>
  <div class=col><div class=panel>
    <div class=row><h2>Sessions</h2><div>
      ...
      <button class="btn" onclick="seed()">Seed Demo</button>
    </div></div>
    <div id=sessions></div>
  </div></div>
  <div class=col><div class=panel>
...
async function seed(){await fetch('/api/seed-demo',{method:'POST'}); await loadSessions()}
...
async function loadSessions(){
  ...
  const wrap=el('div'); (j.sessions||[]).forEach(s=>{
    const card=el('div',{class:'item',onclick:()=>open(s.session_id)},[
      el('div',{class:'row'},[
    ...
    wrap.append(card);
  }); $("#sessions").innerHTML=""; $("#sessions").append(wrap);
}

async function open(id){
  current=id; const r=await fetch('/api/sessions/'+id); if(!r.ok){flash("Load failed",false);return}
  ...
  const d=el('div',{},[
    ...
    el('div',{},[el('button',{class:'btn good',onclick:saveOverview},'Save Overview')]),
    el('h3',{},'Notes'),
    el('div',{},[el('textarea',{id:'notes',rows:'6'},s.notes||'')]),
    el('div',{},[el('button',{class:'btn good',onclick:saveNotes},'Save Notes')]),
    el('h3',{},'Quote'),
    renderQuote(s.quote||[])
  ]);
...
function renderQuote(items){
  ...
  function add(it={}){ const tr=el('tr',{},[
    ...
    el('td',{},el('button',{class:'btn bad',onclick:()=>tr.remove()},'✖'))
  ]); body.append(tr) }
  items.forEach(add);
  const bar=el('div',{class:'row'},[
    el('button',{class:'btn',onclick:()=>add({description:'',quantity:1,unit_price:0})},'+ Add Item'),
    el('button',{class:'btn good',onclick:saveQuote},'Save Quote')
  ]);
  return el('div',{},[t,bar]);
}
...
async function saveOverview(){
  ...
  const r=await fetch('/api/sessions/'+current,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  if(!r.ok){flash('Save failed',false);return} flash('Overview saved'); await open(current); await loadSessions();
}
async function saveNotes(){
  if(!current) return;
  const body={notes:$("#notes").value};
  const r=await fetch('/api/sessions/'+current,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  if(!r.ok){flash('Notes failed',false);return} flash('Notes saved'); await open(current);
}
async function saveQuote(){
  ...
  if(!r.ok){flash('Quote failed',false);return} flash('Quote saved'); await open(current); await loadSessions();
}
loadSessions();
</script>
"""
@app.get("/admin")
def admin(): return (ADMIN, 200, {"Content-Type":"text/html; charset=utf-8"})

# helpful redirect
@app.get("/")
def root(): return redirect("/admin", code=302)
